...
phenology = read_excel("compiled_data/Phenology_All Sites_ 14-16_FIELD Data.xlsx",
...
##2014 trial no records, TOP trials not well recorded. Discard them.
phenology = phenology %>% 
  select(-Sort) %>%  
  distinct() %>% 
  filter(Year != "2014",  
