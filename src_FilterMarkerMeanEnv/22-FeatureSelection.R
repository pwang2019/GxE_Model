######################################################################################################################################
##Feature selection ----------------- feature selection only on Env factors, not on SNPs
...
predictors = features_df[, -c(1:5)]		#1st 5 columns are site info and 2 phenotypes ZS49 and yield
...
############################################
#ZS49 RFE --------------- normalised Predictors, 18 top var; un-normalised predictors, 67 top vars
na.rows = which(is.na(features_df$ZS49.days))
rfe_results_ZS49_FMarker_AEnv = rfe(predictors[-na.rows,], 
                        y = features_df$ZS49.days[-na.rows], 
                        sizes = subsetSizes,
                        rfeControl = recursive_feature_elim_control)

############################################						
#GrYld RFE --------------- normalised Predictors, 91 top var; un-normalised predictors, 14 top vars
na.rows = which(is.na(features_df$GrYld.kg.ha))
rfe_results_GrYld_FMarker_AEnv = rfe(predictors[-na.rows,], 
...
##ZS49
#RFE plot
rfe_FMarker_AEnv_ZS49 = ggplot(rfe_results_ZS49_FMarker_AEnv) + theme_cowplot(12)
ggsave("outputs/plots/rfe_FMarker_AEnv_ZS49.pdf", rfe_env_ZS49, width = 6, height = 4)

#Importance
imp_env_ZS49 = ggplot(data = rfe_results_ZS49_FMarker_AEnv$fit$importance %>% as.data.frame() %>% rownames_to_column()) +
...
ggsave("outputs/plots/Imp_FMarker_AEnv_ZS49.pdf", imp_env_ZS49, width = 6, height = 4)

####################
##Yld
#RFE plot
rfe_env_GrYld = ggplot(rfe_results_GrYld_FMarker_AEnv) + theme_cowplot(12) #+
ggtitle("Recursive Feature Elimination for Environmental Variables")
ggsave("outputs/plots/rfe_FMarker_AEnv_GrYld.pdf", rfe_env_GrYld, width = 6, height = 4)

#Importance
imp_env_GrYld = ggplot(data = rfe_results_GrYld_FMarker_AEnv$fit$importance %>% as.data.frame() %>% rownames_to_column() %>% top_n(25, `%IncMSE`)) +
  geom_bar(mapping = aes(y = `%IncMSE`, x = reorder(rowname, `%IncMSE`)), stat = "identity") +
  coord_flip() + ylab("Importance (% Increase in MSE When Omitted)") + xlab("Environmental Covariate") + theme_cowplot(12)
ggsave("outputs/plots/Imp_FMarker_AEnv_GrYld.pdf", imp_env_GrYld, width = 6, height = 4)

######################################################################################################################################
#now combine key features for the tested traits
shared_predictors = rfe_results_ZS49_FMarker_AEnv$optVariables %in% rfe_results_GrYld_FMarker_AEnv$optVariables
combined_predictors = c(rfe_results_ZS49_FMarker_AEnv$optVariables, rfe_results_GrYld_FMarker_AEnv$optVariables) %>% unique()
